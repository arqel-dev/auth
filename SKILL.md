...
> **Authentication (login/registro/forgot-password) é opt-in dentro deste pacote.** `arqel-dev/auth` ship páginas Inertia-React bundled (login, registro, forgot/reset-password e verificação de e-mail), habilitadas via `Panel::login()`/`registration()`/`passwordReset()` — AUTH-006/007/008 entregues (ver Status abaixo), equivalente ao que Filament/Nova oferecem out-of-the-box. Como alternativa, a app pode delegar a um starter kit Laravel (Breeze/Jetstream/Fortify) — o `arqel new` CLI instala Breeze + React + Inertia por default. Ver `apps/docs/guide/authentication.md`.
...
- `Panel::passwordReset()/passwordResetEnabled()/passwordResetExpirationMinutes()/forgotPasswordUrl()` — fluent API opt-in. `passwordResetExpirationMinutes` ajusta `auth.passwords.{broker}.expire` em runtime, onde `{broker}` é o broker do guard do painel (default `users`) — ver nota #191 abaixo
...
- `Arqel\Auth\Routes::register(?Panel)` — registo idempotente. Quando o `?Panel` é omitido, **auto-detecta** o panel via `PanelRegistry::getCurrent()` desde que o panel tenha `->login()` activo. Pula quando o host já tem rota nomeada `login` (Breeze/Jetstream/Fortify) para coexistir com starter kits. As rotas GET/POST de login e registro aplicam o middleware `guest` (igual às rotas de reset), redirecionando usuários já autenticados para fora do formulário.
...
- **Guard do painel** (#139, #153): todo o fluxo de auth bundled (login/logout/register/email-verify/password-reset + `EnsureUserCanAccessPanel`) autentica contra o guard resolvido por `Concerns\ResolvesPanelGuard` — single source of truth: `PanelRegistry::getCurrent()?->getAuthGuard()`, com fallback para `config('arqel.auth.guard', 'web')` e depois `web`. As rotas emitem `auth:{guard}`/`guest:{guard}`. Um painel com `->authGuard('admin')` passa a autenticar/gatear no guard `admin`; sem `authGuard`, tudo permanece em `web` (equivalente a `auth`/`guest` puros). O `EmailVerificationController` (#153) resolve o usuário via `Auth::guard($this->resolvePanelGuard())->user()` em `notice()`/`resend()`; o `verify()` injeta `Http\Requests\PanelEmailVerificationRequest`, subclasse de `Illuminate\Foundation\Auth\EmailVerificationRequest` que sobrescreve `user()` para defaultar ao guard do painel — necessário porque `authorize()`/`fulfill()` resolvem o usuário **antes** do corpo do controller. Não confie na promoção implícita do guard pelo middleware `auth:{guard}` (`shouldUse`): resolva sempre o guard do painel explicitamente. **Password broker (#191):** `ResolvesPanelGuard::resolvePasswordBroker()` mapeia o guard do painel → seu provider (`config("auth.guards.{guard}.provider")`) → o broker em `config('auth.passwords')` cujo `provider` casa (fallback: nome do provider, depois `'users'`). `ForgotPasswordController`/`ResetPasswordController` usam `Password::broker($this->resolvePasswordBroker())` em vez do broker default; `RegisterRequest` deriva a tabela do email-unique do model do provider do guard (espelhando `RegisterController::resolveUserModel()`); e `Panel::passwordResetExpirationMinutes()` escreve no broker resolvido. Default `web`→`users`→broker `users` permanece byte-idêntico
